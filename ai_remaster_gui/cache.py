...
def delete_cache_file(path_text: str) -> dict:
    ...
    app_context.APP.log.append(f"Deleted cached file: {rel(path)}")
...
def delete_cache_category(category_key: str) -> dict:
    if category_key == "all":
        ...
        app_context.APP.log.append(f"Cleared all ARP cache categories: {total['deleted']} files, {human_size(total['bytes'])}.")
        return total
    ...
    for file in cache_category_files(category):
        ...
        try:
            ...
            app_context.APP.log.append(f"Could not delete cached file {rel(path)}: {exc}")
            continue
        deleted += 1
        bytes_deleted += size

    clean_empty_cache_dirs(category)
    app_context.APP.log.append(f"Cleared {category['title']}: {deleted} files, {human_size(bytes_deleted)}.")
