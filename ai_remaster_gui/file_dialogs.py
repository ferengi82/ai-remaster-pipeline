...
def remember_browse_dir(selected: str) -> None:
    if not selected or "app_context.APP" not in globals():
        return
    path = resolve(selected)
    folder = path if path.is_dir() else path.parent
    if not folder.exists():
        return
    app_context.APP.settings.setdefault("global", {})["last_browse_dir"] = str(folder)
    app_context.APP.save()
...
{show_script}
"""
    if "app_context.APP" in globals():
        app_context.APP.log.append(f"Opening Windows browse dialog for {kind}: {initial_dir}")
    ...
    if selected:
        app_context.APP.log.append(f"Browse selected: {selected}")
    else:
        app_context.APP.log.append("Browse cancelled.")
